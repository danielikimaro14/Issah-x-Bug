function getTimezone() {
  try { return require('./botConfig').getTimezone() || 'Africa/Nairobi' } catch { return 'Africa/Nairobi' }
}
...
exports.getTime = (format, date) => {
    if (date) {
        return moment(date).tz(getTimezone()).locale('en').format(format)
    } else {
        return moment.tz(getTimezone()).locale('en').format(format)
    }
}
...
exports.tanggal = (numer) => {
    ...
    const time = moment.tz(getTimezone()).format('DD/MM HH:mm:ss');
    const d = new Date();
    const locale = 'en';
...
exports.jam = (numer, options = {}) => {
    ...
    let timeZone = options?.timeZone ? options.timeZone : getTimezone()
    let jam = options?.timeZone ? moment(numer).tz(timeZone).format(format) : moment(numer).tz(getTimezone()).format(format)
