...
async function sticker3(img, url, packname, author) {
  if (!url) {
    // `uploadFile` was never defined in this module (ReferenceError on every call).
    // Write the buffer to tmp and use the repo's existing uploader instead.
    const inp = path.join(tmp, +new Date + '.png')
    await fs.promises.writeFile(inp, img)
    try { url = await UploadFileUgu(inp) } finally { fs.promises.unlink(inp).catch(() => {}) }
  }
