const fs = require('fs')
const { STORE_FILE } = (() => { try { return require('./paths') } catch { return { STORE_FILE: require('path').join(__dirname, '..', 'baileys_store.json') } } })()
...
const store = {
    ...
    readFromFile(filePath = STORE_FILE) {
        try {
            if (fs.existsSync(filePath)) {
                ...
                if (!raw.trim()) return   // truncated file from an interrupted write
    ...
    writeToFile(filePath = STORE_FILE) {
        try {
            ...
            // tmp + rename: an interrupted in-place write is what left several
            // data/*.json files truncated to 0-1 bytes
    ...
    bind(ev) {
        ...
        // contacts.upsert fires on initial sync — full contact list from WhatsApp
        ev.on('contacts.upsert', (contacts) => {
            contacts.forEach(upsertContact)
        })

        // contacts.update fires when a single contact changes their name/profile
